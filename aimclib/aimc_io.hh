/* 
 * Copyright EPFL 2021
 * Joshua Klein
 * 
 * This file contains wrappers for interfacing the AIMC core via memory device
 * address.  The addressing used is for use with the semantics defined in the
 * gem5 AIMC wrapper model.
 *
 * NOTE: Since we do not yet model parameter read/write, we just intrinsics for
 * now.
 *
 */

#ifndef __AIMC_IO_HH__
#define __AIMC_IO_HH__

#include <errno.h>      // perror
#include <sys/mman.h>   // mmap
#include <unistd.h>     // sysconf
#include <fcntl.h>      // open

#define AIMC_IO_ADDR            0x08000000
#define AIMC_IO_RANGE           0x1000
#define AIMC_IO_CORENO_ADDR     (AIMC_IO_ADDR + 0x0000)
#define AIMC_IO_QUEUE_ADDR      (AIMC_IO_ADDR + 0x0100)
#define AIMC_IO_DEQUEUE_ADDR    (AIMC_IO_ADDR + 0x0200)
#define AIMC_IO_PROCESS_ADDR    (AIMC_IO_ADDR + 0x0300)

// Set these pointers first for performing AIMC operations.
void * aimcVirtAddrQueue;
void * aimcVirtAddrDequeue;
void * aimcVirtAddrProcess;


// Based on spm memory allocation from CNN solver.  Helper function gets
// pointer for specific AIMC operation for given AIMC core number.  This
// should be called for application setup.
void *
aimcGetAddrPtr(uint64_t mem_addr)
{
    uint64_t alloc_mem_size, page_mask, page_size;
    void * mem_pointer, virt_addr;

    page_size = sysconf(_SC_PAGESIZE);
    alloc_mem_size = (((AIMC_IO_RANGE / page_size) + 1) * page_size);
    page_mask = (page_size - 1);

    int mem_dev = open("/dev/mem", O_RDWR | O_SYNC);
    if(mem_dev == -1)
        perror("Cannot open /dev/mem \n");

    mem_pointer = mmap(NULL,
            alloc_mem_size,
            PROT_READ | PROT_WRITE,
            MAP_SHARED,
            mem_dev,
            (mem_addr & ~page_mask)
    );

    if(mem_pointer == MAP_FAILED)
        perror("Cannot MAP \n");

    printf("Memory Mapped \n");
    virt_addr = (mem_pointer + (mem_addr & page_mask));

    return virt_addr;
}

/* CM Core Process (MVM)
 * Arguments: None.
 */
inline void
aimcProcess(int tid = 0)
{
    aimcVirtAddr + AIMC_IO_PROCESS_ADDR + tid = 1;

    return;
}

/* CM Core Input Memory Queue
 * Queueing arguments:
 * -- rm = QUEUE_MAX input values packed as <val7, val6, ..., val0> for
 *         queueing.
 */
inline void
aimcQueue(uint64_t rm, int tid = 0)
{
    aimcVirtAddr + AIMC_IO_QUEUE_ADDR + tid = (uint32_t)(rm & 0xffffffff);

    return;
}

/* CM Core Output Memory Dequeue
 * Queueing arguments:
 * -- rd = QUEUE_MAX output values packed as <val7, val6, ..., val0>.
 */
inline uint64_t
aimcDequeue(int tid = 0)
{
    uint64_t res = ((uint32_t *)(aimcVirtAddr + AIMC_IO_DEQUEUE_ADDR + tid))[0];

    return res;
}

/* CM Core Parameter Read
 * Instruction format: |____Opcode___|__rm__|_?|__ra__|__rn__|__rd__|
 * Bits:               |31_________21|20__16|15|14__10|9____5|4____0|
 * Binary layout:      |0100_0001_000|0_1000|_1|000_00|01_001|0_1010|
 * Hex layout:         |__4____1____0|____8_|__|_8____|1____2|____A_|
 * gem5 variables:     |_____________|_Op264|__|_Op364|_Op164|Dest64|
 *
 * Queueing arguments:
 * -- rd = Parameter value.
 * -- rm = Parameter x index.
 * -- rn = Parameter y index.
 */
inline uint64_t
aimcParamRead(uint64_t rm, uint64_t rn, uint64_t ra, int tid = 0)
{
    uint64_t res;

    __asm__ volatile(
        "MOV X9, %[input_j];"
        "MOV X7, %[input_k];"
        ".long 0x4108812A;"
        "MOV %[output], X10;"
        : [output] "=r" (res)
        : [input_j] "r" (rm), [input_k] "r" (rn)
        : "x7", "x9", "x10"
    );

    return res;
}

/* CM Core Parameter Write
 * Instruction format: |____Opcode___|__rm__|_?|__ra__|__rn__|__rd__|
 * Bits:               |31_________21|20__16|15|14__10|9____5|4____0|
 * Binary layout:      |0100_0001_000|0_1000|_0|001_11|01_001|0_1010|
 * Hex layout:         |__4____1____0|____8_|__|_1____|D____2|____A_|
 * gem5 variables:     |_____________|_Op264|__|_Op364|_Op164|Dest64|
 *
 * Queueing arguments:
 * -- rd = Success code.
 * -- rm = Parameter x index.
 * -- ra = Parameter value.
 * -- rn = Parameter y index.
 */
inline uint64_t
aimcParamWrite(uint64_t rm, uint64_t rn, uint64_t ra, int tid = 0)
{
    uint64_t res;

    __asm__ volatile(
        "MOV X8, %[input_i];"
        "MOV X9, %[input_j];"
        "MOV X7, %[input_k];"
        ".long 0x41081D2A;"
        "MOV %[output], X10;"
        : [output] "=r" (res)
        : [input_i] "r" (ra), [input_j] "r" (rm), [input_k] "r" (rn)
        : "x7", "x8", "x9", "x10"
    );

    return res;
}

#endif // __AIMC_IO_HH__
